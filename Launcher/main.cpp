...
int main(int argc, char* argvp[])
{
	...
	if (false == shader.Initialize("shaders/block.vert", "shaders/block.frag"))
	{
	...
	try
	{
		...
		if (false == game.GetNetworkClient().Login(server, port, username, password, UpdateMethod::Manual))
		{
	...
	while (false == glfwWindowShouldClose(window))
	{
		...
		auto player_chunk = game.GetNetworkClient().GetWorld()->GetChunk(ToVector3i(game.GetPosition()));
...
GLFWwindow* InitializeWindow()
{
	...
	GLFWwindow* window = glfwCreateWindow(1920, 1080, "Terracotta", nullptr, nullptr);
