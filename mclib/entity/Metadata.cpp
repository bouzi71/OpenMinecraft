...
DataBuffer EntityMetadata::SlotType::Serialize(Version protocolVersion)
{
...
void EntityMetadata::SlotType::Deserialize(DataBuffer& in, Version protocolVersion)
{
...
DataBuffer& operator<<(DataBuffer& out, const EntityMetadata& md)
{
	for (std::size_t i = 0; i < md.MetadataCount; ++i)
	{
		...
		if (md.m_ProtocolVersion <= Version::Minecraft_1_12_2 && type >= 5)
		{
		...
		switch (type)
		{
			case EntityMetadata::DataType::Byte:
				...
				break;
			case EntityMetadata::DataType::CVarInt:
...
EntityMetadata::EntityMetadata(Version protocolVersion)
